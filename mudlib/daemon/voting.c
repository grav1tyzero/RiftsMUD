...
int register_vote(string who, string whom, string v_class) {
    if(!elir) return NOT_ELECTION_DAY;
    if(!who || !whom || !v_class) return VOTE_ERROR;
    if(member_array(v_class, ELECTIONS) == -1) return VOTE_ERROR;
    if(current_class != v_class) {
...
int register_candidate(string who, string v_class) {
    if(!elir) return NOT_ELECTION_DAY;
    if(!who || !v_class) return VOTE_ERROR;
    if(member_array(v_class, ELECTIONS) == -1) return VOTE_ERROR;
    if(current_class != v_class) {
	current_class = v_class;
        seteuid(UID_VOTESAVE);
...
string *query_candidates(string v_class) {
    if(!elir) return ({});
    if(member_array(v_class, ELECTIONS) == -1) return ({});
    if(current_class != v_class) {
	current_class = v_class;
        seteuid(UID_VOTESAVE);
	restore_object(DIR_VOTES+"/"+current_class);
