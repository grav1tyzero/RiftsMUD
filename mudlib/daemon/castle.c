...
void create() {
    ...
    seteuid(getuid());
    __Rooms = ([]);
    i = sizeof(ls = get_dir(DIR_CASTLES_HM+"/"));
    while(i--)
      catch(call_other(DIR_CASTLES_HM+"/"+ls[i]+"/castle", "init_castle"));
}
...
object query_room(string who, string save_file) {
    ...
    if(!__Rooms[who][save_file]) {
        ...
        seteuid(UID_HM);
        __Rooms[who][save_file]->setup(DIR_CASTLES_HM+"/"+who+"/"+save_file);
        seteuid(getuid());
    }
...
void add_room(string who, string long, string short, string *first_exit, string dir) {
    ...
    if(file_size(DIR_CASTLES_HM+"/"+who+"/") != -2) return;
    if(!__Rooms[who]) __Rooms[who] = ([]);
    __Rooms[who][save_file="room"+sizeof(get_dir(DIR_CASTLES_HM+"/"+who+"/"))] =
      new(OB_HM_ROOM);
