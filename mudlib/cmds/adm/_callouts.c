...
int cmd_callouts(string str)
{
  ...
  write(sprintf("%-36s%-18s%7s%10s\n", "Object", "Function", "Delay", "Impl"));
  write(sprintf("%-36s%-18s%7s%10s\n", "------", "--------", "-----", "----"));
  ...
  for(i = 0; i < sizeof(crap); i++)
  {
    ...
    if(sizeof(element) != 3)
    {
      write("Size problem " + sizeof(element) + " " + sprintf("%O", element) + "\n");
      continue;
    }
    ob = (object)element[0];
...
//  args  = element[3];
    ...
    for(j = strlen(filename); j < 35; j++)
      spaces += " ";
    printing_crap = sprintf("%-36s%-18s%7d%10s", filename,func,delay, "callout");
