...
void save_this_room() {
  ...
  save_object("/d/damned/data/inn_room");
  seteuid(getuid());
  return;
}
...
int query_time_left(string owner) {
  ...
  if(member_array(owner, query_all_owners()) < 0)
    return 0;
  room_file = sprintf("/d/damned/virtual/inn_%d.inn",
...
void move_all_inv(object room_obj) {
  ...
  if(room_obj) {
	...
	inv[i]->move(load_object("/d/damned/inn/gift_shop_store"));
    }
  }
  return;
}
...
int get_next_room() {
  ...
  for(j=0;j < max; j++) {
    num = sprintf("%d%2'0'd", j/ROOMS_PER_FLOOR + 1,
                  j%ROOMS_PER_FLOOR);
    if(!owners || !owners[sprintf("/d/damned/virtual/inn_%s.inn", num)])
      return atoi(num);
  }
  return 0;
}
...
void create() {
  ...
  set("short", "Front Desk of the Blue Dragon Inn");
  set("long",
      "This is the front desk of the Blue Dragon Inn.  "+
	...
	"You see a gift shop to the east.\n");
  set_items( (["sign":
