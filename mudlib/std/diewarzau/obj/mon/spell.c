...
void do_criticals(string *criticals, string target_thing, object caster, object targ) {
    ...
    string *tmp, what, what2, *tmp2, *mesgs;
    object ob;
    object *targ_armour;
    int k, x, l;
    mixed tmp_crits;
...
void begin_casting(object caster, string arg, int power) {
    ...
    if(props["no target"]) xtra_arg = arg;
      ...
      target = arg;
    if((int)caster->query_spell_level(props["name"]) < power) {
...
void do_spell(mapping info) {
    string target, args, targ_limb, *tmp, res;
    object caster, at, caster_env, ob;
    ...
    if(roll > (95 + skill / 20) || roll == 100) {
		...
		(string)caster->query_cap_name()+
		" fumbles "+possessive(caster)+" spell!",
		environment(caster),({ caster }));
		fumble_flag = 1;
	} else if(roll > skill) {
	     message("info","%^CYAN%^Your spell sputters ineffectively.",
		caster);
	     message("info","%^CYAN%^%^BOLD%^"+(string)caster->
		query_cap_name()+"'s spell fails.",
		environment(caster),({ caster }));
	...
	resist = (int)at->query_property("magic resistance") +
	    (stringp(props["save mod"]))?
	    ((int)at->query_stats(props["save mod"])/2):0;
	resist -= 10 + (power - 3) * 16;
	resist -= ((int)caster->query_level() - (int)at->query_level()) * 5;
	if(resist < 0) resist = 0;
	if(random(100) < resist) {
	  if(props["can resist"] == 1) {
...
void spell_func(object caster, object at, int power, string args, int flag) {
	...
	({ caster }));
		...
		if(props["save mod"])
		    resist += (int)inv[k]->query_stats(props["save mod"]) / 2;
		resist -= 10 + (power - 3) * 16;
	...
	case "protection":
	    ...
	    if(stringp(args) && member_array(args,(string *)at->query_limbs()))
		{ caster->add_mp(props["mp cost"]/2);
		tmp_ob->set_limbs(({ args })); }
	    break;	    
    }
