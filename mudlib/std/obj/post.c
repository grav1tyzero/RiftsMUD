...
void start_mail(string str) {
    ...
    message("mail", "Imaginary Intermud Postal Service (IIPS) 2.0"
      "\nDescartes of Borg 1993, 1994 (type \"?\" for help)", this_player());
    message("mail", "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
      "-=-=-=", this_player());
...
void do_groups(string str) {
    ...
    if(!str || str == "") {
      ...
      this_player()->more(explode(format_page(keys(__MudGroups)+keys(__MyGroups),
        4), "\n"), "system", (: call_other, this_object(), "do_mail" :));
      return;
    }
    ...
    switch(str) {
        ...
        default:
          this_player()->more(explode(read_file(wizardp(this_player()) ?
            ...
            (: call_other, this_object(), "do_mail" :));
          return;
    }
    do_mail(0);
}
...
void headers(int x) {
    ...
    for(lines = ({}); x<i; x++) {
      lines +=({ (x+1)+" "+(__BoxInfo[x]["read"] ? "     " :
          "(new)")+(x==__Current ? ">" : (__Delete[x] ? "*" : " "))+
          arrange_string(capitalize(__BoxInfo[x]["from"]), 16)+" "+
          arrange_string(get_header_time(__BoxInfo[x]["date"]), 12)+
          __BoxInfo[x]["subject"] });
    }
    this_player()->more(lines, "mail", (: call_other, this_object(), "do_mail" :));
}

string get_header_time(mixed x) {
    string a, b, c, d, the_ctime;
    int y;
...
void read_mail(int x) {
    ...
    __BoxInfo[x]["read"] = 1;
    this_player()->more(explode(borg, "\n"), "mail",(: call_other,this_object(),"do_mail":));
}
